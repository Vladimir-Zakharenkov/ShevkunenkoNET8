...
public class VideoLinksViewModel
{
    ...
    public string SearchFilter { get; set; } = string.Empty; // фильтр поиска фильмов

    public bool MovieInMainList { get; set; } = true; // true - отображать фильмы с параметром InMainList=true, false - все найденные фильмы

    public bool IsPartsMoreOne { get; set; } = true; // true - ссылка на страницу серий фильма (если фильм многосерийный)
}